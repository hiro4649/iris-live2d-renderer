...
const summary = report.fixOutcome || { fixOutcomeStatus: 'unknown', resolvedFindings: [], unresolvedFindings: [], newRisks: [] };
console.log('== Codex fix outcome tracker ==');
console.log(`fixOutcomeStatus: ${summary.fixOutcomeStatus || summary.status || 'unknown'}`);
console.log(`resolvedFindings: ${(summary.resolvedFindings || []).length}`);
console.log(`unresolvedFindings: ${(summary.unresolvedFindings || []).length}`);
console.log(`newRisks: ${(summary.newRisks || []).length}`);
console.log(`recommendedFollowUp: ${summary.recommendedFollowUp || 'run post-fix verification'}`);
