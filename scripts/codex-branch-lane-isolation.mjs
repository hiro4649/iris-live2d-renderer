...
const report = {
  ...
  ...buildBranchLaneIsolationReport({
    changedFiles: (process.env.CODEX_CHANGED_FILES || '').split(';').filter(Boolean),
    protectedPatch: process.env.CODEX_PROTECTED_PATCH === '1',
    untrackedArtifact: process.env.CODEX_UNTRACKED_ARTIFACT === '1',
...
writeJsonReport(report, 'CODEX_BRANCH_LANE_ISOLATION_REPORT');
exitFor(report);
