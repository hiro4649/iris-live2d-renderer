...
こ�E roadmap は phase-based schedule であり、calendar date は持たなぁE��各 phase は前段の safety boundary と evidence requirement を壊さずに進める、Eatency target は local renderer の engineering target であり、E�E信プラチE��フォーム上�E viewer-visible latency は別途発生する、E

## Phase 0: PR 27 dependency

- Status: PR 27 must remain separate、E
- Scope: cue retention、readiness truthfulness、optional write auth、safe runtime config、E
- Readiness: no runtime readiness claim、E

## Phase 1: REACTION-LATENCY-SPEC1

- Status: current docs-only PR、E
- Scope: reaction latency targets、two-stage response design、transport plan、preload plan、safety boundaries、implementation order、E
- Runtime: no runtime code、E

## Phase 2: RENDERER-CUE-CONTRACT-SAFETY1

- Scope: cue allowlist、unsupported cue safe reject、recovery-required strong motion validation、E
- Outputs: safe cue validator、raw command / model path / endpoint / candidate / world_command rejection tests、E
- Model loading: no real model loading、E

## Phase 3: PUSH-CUE-DELIVERY2

- Scope: cue contract safety merge 後に SSE また�E WebSocket delivery めEdesign / implement する、E
- Goal: browser delivery latency めEpolling-scale から低減する、E
- Boundary: safe output と auth boundaries を維持する、E
- Readiness: real evidence がなぁE��めEreadiness claim なし、E

## Phase 4: PRELOAD-AND-SAFE-ASSET-ROUTE3

- Scope: safe model asset route、preloading strategy、no raw path exposure、E
- Route: unavailable route めEadvertise しなぁE��E
- Tests: model path redaction を検証する、E

## Phase 5: REAL-MODEL-LOAD4

- Scope: real Cubism/model3 load、scene binding、fresh heartbeat、real model evidence、E
- Goal: model_loaded と scene_loaded めEtruthfully に enable する、E
- Production: no production readiness claim、E
...
- Scope: instant nonverbal cue pack、E
- Target: internal event arrival 征E50 to 300 ms local renderer target、E
- Examples: blink_attention、small_nod、soft_smile、surprise_micro、breathing_shift、E

## Phase 7: COMMENT-REACTION-PIPELINE6

- Scope: comment acknowledgement first、speech/TTS second、E
- Targets: visual ack は local comment event 征E300 to 800 ms。spoken response は feasible な場合に 1.5 to 4 seconds、E

## Phase 8: GAME-REACTION-PIPELINE7

- Scope: approved safe game-event reactions、E
- Target: approved local game event 征E50 to 300 ms、E
- Boundary: no game input or OS command、E

## Phase 9: STRONG-MOTION-RECOVERY8

- Scope: laugh_big、surprise_scream、happy_dance、happy_loud_sing、closeup、E
- Recovery: required、E
- Guard: viewer comfort と visibility guard が忁E��、E

## Phase 10: LIVE-READINESS-EVIDENCE9

- Scope: fresh Live2D evidence collector、owner confirmation、go/no-go integration、E
- Boundary: no fixture-to-real promotion、E
- Production: owner confirmation と fresh evidence なしに production go しなぁE��E
